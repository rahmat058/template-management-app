...
interface EditorState {
  ...
  resizeElement: (elementId: string, box: { x: number; y: number; width: number; height: number }) => void
  updateElement: (
    elementId: string,
...
function isPristineUntitledTab(tab: EditorTab): boolean {
  return tab.templateId === null && !tab.isDirty && tab.name === DEFAULT_TAB_NAME
}
...
function replaceActiveTab(tabs: EditorTab[], activeTabId: string, updater: (tab: EditorTab) => EditorTab): EditorTab[] {
  return tabs.map((tab) => (tab.id === activeTabId ? updater(tab) : tab))
}

function updateActivePage(tab: EditorTab, updater: (page: Page) => Page): EditorTab {
  return {
    ...tab,
    isDirty: true,
    document: {
...
function mutateActiveTab(
  ...
  return replaceActiveTab(tabs, activeTabId, (tab) =>
    historyKey ? withHistory(tab, `${tab.id}:${historyKey}`, updater) : updater(tab),
  )
}
...
export const useEditorStore = create<EditorState>((set, get) => {
  ...
  return {
    ...
    openTemplate: (template) => {
      ...
      const { tabs } = get()
      const onlyPristineUntitled = tabs.length === 1 && tabs[0] && isPristineUntitledTab(tabs[0])
    ...
    addElement: (element) => {
      set((state) => ({
        tabs: mutateActiveTab(state.tabs, state.activeTabId, `add:${element.id}`, (tab) => ({
    ...
    moveElement: (elementId, x, y) => {
      set((state) => ({
        tabs: mutateActiveTab(state.tabs, state.activeTabId, `move:${elementId}`, (tab) =>
          updateActivePage(tab, (page) => ({
            ...page,
            elements: page.elements.map((element) => {
              ...
              return {
                ...element,
                x: Math.round(Math.max(0, Math.min(page.width - element.width, x))),
                y: Math.round(Math.max(0, Math.min(page.height - element.height, y))),
              }
            }),
          })),
        ),
    ...
    resizeElement: (elementId, box) => {
      set((state) => ({
        tabs: mutateActiveTab(state.tabs, state.activeTabId, `resize:${elementId}`, (tab) =>
          updateActivePage(tab, (page) => ({
            ...page,
            elements: page.elements.map((element) => {
              if (element.id !== elementId) {
    ...
    updateElement: (elementId, updater, historyKey) => {
      set((state) => ({
        tabs: mutateActiveTab(state.tabs, state.activeTabId, historyKey ?? `element:${elementId}`, (tab) =>
          updateActivePage(tab, (page) => ({
            ...page,
    ...
    updatePage: (updater) => {
      set((state) => ({
        tabs: mutateActiveTab(state.tabs, state.activeTabId, 'page', (tab) => updateActivePage(tab, updater)),
      }))
    },
