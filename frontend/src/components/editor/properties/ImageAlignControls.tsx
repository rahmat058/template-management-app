...
export function ImageAlignControls({ elementId }: { elementId: string }) {
  ...
  const pageWidth = useEditorStore(
    (state) => state.getActivePage()?.width ?? 794,
  );
...
function AlignButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex h-8 flex-1 items-center justify-center text-muted hover:bg-surface-muted hover:text-primary",
      )}
    >
      {children}
    </button>
