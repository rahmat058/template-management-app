...
export function Canvas({ interactive = true, pageIds }: CanvasProps) {
  ...
  return (
    <section className="bg-workspace relative flex min-h-0 min-w-0 flex-1 flex-col">
      <div className="border-border bg-surface absolute top-4 right-4 z-10 flex items-center gap-1 rounded-[8px] border px-1 py-1 shadow-sm">
        <button
          type="button"
          aria-label="Zoom out"
          className="text-muted hover:bg-surface-muted hover:text-foreground flex h-7 w-7 items-center justify-center rounded-[6px]"
          onClick={() => setZoom(Math.max(0.5, Number((zoom - 0.1).toFixed(1))))}>
          <Minus className="h-3.5 w-3.5" />
        </button>
        <span className="text-foreground min-w-[48px] text-center text-[12px] font-medium">
          {Math.round(zoom * 100)}%
        </span>
        <button
          type="button"
