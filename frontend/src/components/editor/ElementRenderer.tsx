  ...
  return (
    <div
      ...
      className={cn(
        ...
        interactive && !selected && 'hover:ring-primary/30 hover:ring-1',
        selected && interactive && 'ring-primary ring-2 ring-offset-2',
      ...
      {...(interactive && !usesDedicatedHandle && mounted ? dragHandleProps : undefined)}>
      {interactive && element.type === 'text' && !element.locked ? (
        <MoveHandle
          dragHandleProps={dragHandleProps}
          className={cn('opacity-0 group-focus-within:opacity-100 group-hover:opacity-100', selected && 'opacity-100')}
        />
      ) : null}
      {element.type === 'text' ? <TextElement element={element} interactive={interactive} /> : null}
      {element.type === 'table' ? (
        <TableElement
          element={element}
