...
function resolveSaveName(
  ...
  const hasAutoloadTemplate = summaries.some(
    (template) => template.name === AUTOLOAD_TEMPLATE_NAME,
  );

  if (
    !hasAutoloadTemplate &&
    (trimmed === "" || trimmed === DEFAULT_TAB_NAME)
  ) {
